...
public class GeneticManager : MonoBehaviour
{
    ...
    public void Death (float fitness, NNet network) //If the car death we create a new car
    {
        ...
        if (currentGenome < population.Length -1)
        {
            ...
            currentGenome++;
            ResetToCurrentGenome();
            genome.text = currentGenome.ToString();
        }
        else
        {
            RePopulate();
    ...
    private void RePopulate()
    {
        ...
        naturallySelected = 0;
        SortPopulation();
        generation.text = currentGeneration.ToString();

        NNet[] newPopulation = PickBestPopulation();

        Crossover(newPopulation); // We reproducte the actual generation with the new generation
        Mutate(newPopulation); // And the genome mutate

        FillPopulationWithRandomValues(newPopulation, naturallySelected);

        population = newPopulation;

        currentGenome = 0; // The genome recome to 0
    ...
    private NNet[] PickBestPopulation() // We take the best genomes to reproduce that
    {
